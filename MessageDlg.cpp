...
BEGIN_MESSAGE_MAP(CMessageDlg, CDialogEx)
	...
	ON_EN_KILLFOCUS(IDC_EDIT_INPUT, &CMessageDlg::OnEnKillFocusEditInput)
	ON_BN_CLICKED(IDC_BUTTON_ADD, &CMessageDlg::OnClickedButtonAdd)
	ON_BN_CLICKED(IDC_BUTTON_DELETE, &CMessageDlg::OnBnClickedButtonDelete)
	ON_BN_CLICKED(IDC_BUTTON_SAVE, &CMessageDlg::OnBnClickedButtonSave)
END_MESSAGE_MAP()
...
BOOL CMessageDlg::PreTranslateMessage(MSG* pMsg)
{
	...
	{
		if (::GetFocus() == GetDlgItem(IDC_EDIT_INPUT)->GetSafeHwnd())
		{
			return TRUE;
...
LRESULT CMessageDlg::OnReceiveMessage(WPARAM wParam, LPARAM lParam)
{
...
void CMessageDlg::OnEnKillFocusEditInput()
{
	OnClickedButtonAdd();
}
...
{
	CString inputText;
	GetDlgItemText(IDC_EDIT_INPUT, inputText);
	if (!inputText.IsEmpty()) {
		COleDateTime currentTime = COleDateTime::GetCurrentTime();
		CString dateTimeStr = currentTime.Format(_T("[%Y-%m-%d %H:%M:%S] "));
		CString outputText = dateTimeStr + inputText;


		int index = GetDlgItem(IDC_LIST_OUTPUT)->SendMessage(LB_ADDSTRING, 0, (LPARAM)(LPCTSTR)outputText);

		GetDlgItem(IDC_LIST_OUTPUT)->SendMessage(LB_SETCURSEL, index, 0);
		m_editInput.SetWindowText(_T(""));
	}
}
void CMessageDlg::OnBnClickedButtonDelete()
{
	m_listOutput.ResetContent();
}
BOOL CMessageDlg::OnEditEnterKey()
{
	OnClickedButtonAdd();
	return TRUE;
}
void CMessageDlg::OnBnClickedButtonSave()
{
	CStdioFile file;
